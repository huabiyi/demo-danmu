...
//Vue-loader 15.*之后的版本，vue-loader使用需要伴生VueLoaderPlugin 	//*************** */ vue 全家桶添加loader
...
// let jsEntries = getEntry("./src/index.js");
let entPages = Object.keys(getEntry("./src/views/*"));
entPages.forEach(function(dirname){
	let pages2 = Object.keys(getEntry("./src/views/"+dirname+"/*.html"));
	pages2.forEach((pathname)=> {
		jsEntries[dirname+"_"+pathname] = "./src/js/entry/index.js";
    });
})
...
let pages = Object.keys(getEntry("./src/views/*"));
pages.forEach(function (dirname) {
	...
	pages2.forEach((pathname)=> {
      let conf = {
        inject: true,
        chunksSortMode: 'manual',
        filename: dirname+"/"+pathname +'.html', //生成的html存放路径，相对于path
        template: './src/views/'+ dirname+'/'+pathname +'.html', //ejs模板路径,前面最好加上loader用于处理
        chunks : ['vendor', dirname+'_'+pathname]
      };
	    config.plugins.push(new HtmlWebpackPlugin(conf));
    });
})
